...
**一轮（round）** = 最多 N 次讨论，直到双方达成共识，然后 B 修代码

```
第1次讨论：A 审查代码，提出问题  →  B 阅读 A 的发现并逐条验证
第2次讨论：A 回应 B 的验证意见  →  B 再次回应 A               ← 可达成共识
第3次讨论：A 进一步讨论          →  B 进一步讨论               ← 可达成共识
...
第N次讨论：最后一次机会          →                             ← 可达成共识
```

**达成共识的条件**：A 和 B **都**在同一次讨论中明确写出共识关键词（如 `结论：同意` / `CONCLUSION: I agree`）。仅靠第1次讨论几乎不可能达成共识——因为 A 在做初始审查时不会说"同意"，B 在做验证时也未必会用关键词。**实际达成共识最少需要 2 次讨论**（exchange 1：A提问题+B验证；exchange 2：A表示同意+B表示同意）。

因此：`max_exchanges` 建议至少设为 **3~5**，给双方足够的对话空间。
